...
EPS = 1e-4
quality_discrim_loss = tf.reduce_mean(-(tf.log(predict_quality_real + EPS) + tf.log(1 - predict_quality_fake + EPS)))/10
gen_quality_loss = tf.reduce_mean(-tf.log(predict_quality_fake + EPS))/10
...
reconstruction_loss = tf.reduce_mean(tf.abs(tf.expand_dims(input_images,1) - fake_output_of_the_original_type2))
...
def save_images(src,spectrum,batch_number):
    imsave(np.concatenate((src,spectrum),axis=0), [1+number_of_types,1],'train/{:06d}.png'.format(batch_number))
...
for i in range(50001):
    ...
    if i % update_freq == 0:
        ...
        print(gs)
    if i % save_freq == 0:
        spectrum = unnormalize(results['spectrum'][0])
        src = np.expand_dims(ii[0],0)
